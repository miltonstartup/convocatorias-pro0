-- Script completo de configuración de base de datos ConvocatoriasPro
...
DROP POLICY IF EXISTS "Users can view own convocatorias" ON convocatorias;
CREATE POLICY "Users can view own convocatorias" ON convocatorias
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own convocatorias" ON convocatorias;
CREATE POLICY "Users can insert own convocatorias" ON convocatorias
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own convocatorias" ON convocatorias;
CREATE POLICY "Users can update own convocatorias" ON convocatorias
  FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own convocatorias" ON convocatorias;
...
DROP POLICY IF EXISTS "Users can view own rastreo" ON rastreo_automatico;
CREATE POLICY "Users can view own rastreo" ON rastreo_automatico
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can manage own rastreo" ON rastreo_automatico;
...
('free', 'Plan Gratuito', 0, '#', 'Plan básico gratuito', 
  '["Hasta 5 convocatorias", "Dashboard básico", "Calendario simple", "3 días de prueba Pro"]'),
('pro_monthly', 'Pro Mensual', 8990, 'https://mpago.la/PLACEHOLDER_MONTHLY', 'Plan Pro con facturación mensual', 
  '["Convocatorias ilimitadas", "IA para parsing", "Exportación PDF/CSV", "Rastreo automático", "Recomendaciones personalizadas", "Soporte prioritario"]'),
('pro_annual', 'Pro Anual', 84990, 'https://mpago.la/PLACEHOLDER_ANNUAL', 'Plan Pro anual con descuento', 
  '["Todo lo del Pro Mensual", "2 meses gratis", "Funciones beta anticipadas", "1 colaborador incluido", "Badge de usuario Pro"]')
...
-- 15. Insertar datos de prueba (opcional)
INSERT INTO convocatorias (user_id, nombre_concurso, institucion, fecha_cierre, estado, tipo_fondo, area)
SELECT 
  auth.uid(), 
  'CORFO Innovación Digital 2025', 
  'CORFO', 
...
AND NOT EXISTS (SELECT 1 FROM convocatorias WHERE user_id = auth.uid());

-- Finalización del script
SELECT 'Base de datos configurada exitosamente. Todas las tablas, índices, políticas RLS y triggers han sido creados.' as status;
